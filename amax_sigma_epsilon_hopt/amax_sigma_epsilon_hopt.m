%this code simulate the persistence of species in the parameter space (bmax,theta))
%for different values of epsilon and h_opt
...
%h_opt=0.1; % INITIAL HANDDLING TIME -- 0.3 Dee LE 2020
...
epsilon=[0.02, 0.03, 0.04, 0.05]; %High emissions
%epsilon=[0.005, 0.01, 0.015, 0.02]; %Low emissions
...
R=cell(length(h_opt),length(epsilon));
...
for l=1:length(h_opt)
for k=1:length(epsilon)
for i=1:m1
for j=1:m2

   
P=[bmax theta m0 alpha gamma c Topt_x_0 mu epsilon(k) Tmax Gx amax(i) phi h_opt(l) p q0 delta beta sigma(j) Topt_y_0 eta Gy];
%% Initial conditions:
...
if metric1_x(i,j)>umbral1 && metric2_x(i,j)>umbral2 || metric1_y(i,j)>umbral1 && metric2_y(i,j)>umbral2 || (metric3_x(i,j)>0.1 && metric1_x(i,j)>umbral1 ) || (metric3_y(i,j)>0.1 && metric1_y(i,j)>umbral1)
...
contourLevels = [0, 2, 1, 3]; % Para tus datos discretos de 0 a 3
